...
function kupayPDPCheckout() {
  ...
  if (!kupayDisabledButton) {
    kupayBuildModal(iframeUrl)
  } else {
    showTooltip()
  }
}
...
console.log(
  "KUPAY_WOOCOMMERCE_VERSION: ",
  document.getElementById("kupay-woocommerce-version").value
)

function kupayBuildModal(iframeUrl) {
  const modal = document.createElement("div")
  modal.id = "kupay-modal"
  modal.className = "kupay-modal"
  modal.setAttribute("role", "dialog")
  modal.setAttribute("aria-labelledby", "kupay-modal-title")
  modal.setAttribute("aria-hidden", "false")

  // insert iframe into modal
  const iframe = document.createElement("iframe")
  iframe.id = "kupay-iframe"
  ...
  overlay.id = "kupay-overlay"
  overlay.className = "kupay-overlay"

  document.body.appendChild(overlay)

  // create close modal icon
  const closeModal = document.createElement("button")
  closeModal.id = "kupay-close-modal"
  ...
  // toggle classList show modal
  modal.classList.toggle("show")
}

function showTooltip() {
  const tooltip = document.querySelector(".kupay-tooltip")
